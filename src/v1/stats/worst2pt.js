// src/stats/worst2pt.js
// Bottom-10 teams by 2P%
...
export async function getWorst2pt({ seasoncode = DEFAULT_SEASON, minAttempts = 50 } = {}) {
    console.log(`[W2PT] Start for ${seasoncode}`);
    ...
    let rows = all.filter(r => r.att >= thr).sort((a, b) => a.pct - b.pct).slice(0, 10);
    if (rows.length === 0 && all.length > 0) rows = all.sort((a, b) => a.pct - b.pct).slice(0, 10);
    return rows;
}

export function buildWorst2ptEmbed(rows, { seasoncode, minAttempts }) {
    if (!rows?.length) {
        return { title: 'Bottom 10 — 2-Point Percentage', description: `No data for **${seasoncode}** (min ${minAttempts} 2PA).`, timestamp: new Date().toISOString() };
    }
    const lines = rows.map((r, i) => `${i + 1}. **${r.teamName}** — ${(r.pct * 100).toFixed(1)}% (${r.made}/${r.att})`);
    return { title: 'Bottom 10 — 2-Point Percentage', description: [`Season: **${seasoncode}** • Min Attempts: **${minAttempts}**`, '', ...lines].join('\n'), timestamp: new Date().toISOString() };
}

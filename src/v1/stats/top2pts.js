// src/stats/top2pt.js
// Top-10 teams by 2P% from box.Stats["0|1"].PlayersStats[*].FieldGoalsMade2 / FieldGoalsAttempted2
...
const keysOf = (o) => (o && typeof o === 'object' ? Object.keys(o) : null);
...
function sumPlayersFG2(players) {
    if (!Array.isArray(players)) return { made: 0, att: 0, count: 0 };
    let made = 0, att = 0, count = 0;
    for (const p of players) {
        const m = num(p?.FieldGoalsMade2 ?? p?.fieldGoalsMade2);
        const a = num(p?.FieldGoalsAttempted2 ?? p?.fieldGoalsAttempted2);
...
export async function getTop2pt({ seasoncode = DEFAULT_SEASON, minAttempts = 50 } = {}) {
    console.log(`[2PT] Start computation for ${seasoncode}`);
    ...
    console.log(`[2PT] Schedule loaded, total games: ${games.length}`);

    const now = Date.now();
    const past = games.filter((g) => { const dt = getGameDate(g); return dt && dt.getTime() < now; });
    console.log(`[2PT] Past games: ${past.length}`);

    const agg = new Map();
    let fetched = 0, ok = 0, used = 0;
    ...
    for (const g of past) {
        ...
        fetched++;
        let box;
        try { box = await fetchAny(url); ok++; } catch (e) { console.log('[2PT] box fetch fail', gc, e.message); continue; }
        ...
        const A = sumPlayersFG2(a?.PlayersStats);
        const B = sumPlayersFG2(b?.PlayersStats);
    ...
    console.log(`[2PT] boxes: ${fetched}, ok: ${ok}, used: ${used}, teams: ${agg.size}`);
...
export function buildTop2ptEmbed(rows, { seasoncode, minAttempts }) {
    if (!rows?.length) {
        return { title: 'Top 10 — 2-Point Percentage', description: `No data for **${seasoncode}** (min ${minAttempts} 2PA).`, timestamp: new Date().toISOString() };
    }
    const lines = rows.map((r, i) => `${i + 1}. **${r.teamName}** — ${(r.pct * 100).toFixed(1)}% (${r.made}/${r.att})`);
    return { title: 'Top 10 — 2-Point Percentage', description: [`Season: **${seasoncode}** • Min Attempts: **${minAttempts}**`, '', ...lines].join('\n'), timestamp: new Date().toISOString() };
}

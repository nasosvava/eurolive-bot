// src/stats/worstft.js
// Bottom-10 teams by FT%
...
export async function getWorstFT({ seasoncode = DEFAULT_SEASON, minAttempts = 50 } = {}) {
    console.log(`[WFT] Start for ${seasoncode}`);
...
export function buildWorstFTEmbed(rows, { seasoncode, minAttempts }) {
    if (!rows?.length) {
        return { title: 'Bottom 10 — Free-Throw Percentage', description: `No data for **${seasoncode}** (min ${minAttempts} FTA).`, timestamp: new Date().toISOString() };
    }
    const lines = rows.map((r, i) => `${i + 1}. **${r.teamName}** — ${(r.pct * 100).toFixed(1)}% (${r.made}/${r.att})`);
    return { title: 'Bottom 10 — Free-Throw Percentage', description: [`Season: **${seasoncode}** • Min Attempts: **${minAttempts}**`, '', ...lines].join('\n'), timestamp: new Date().toISOString() };
}
